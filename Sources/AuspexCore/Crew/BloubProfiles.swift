...
//
// Radial profiles r(theta) read off the reference video at the pixel.
// theta = 0 points right and grows clockwise (y down). Unit: the resting
// ball's radius = 1.
//
// bloub generates this file with `tools/extract-profiles.py` from the video's
// frames and forbids editing it by hand. The numbers below are transcribed
// from that generated file unchanged; the same rule applies here. To change
// them, regenerate them over there and transcribe again.

/// The three silhouettes that cannot be built analytically, and the angular
/// sampling every silhouette in the engine shares.
public enum BloubProfiles {
    /// How many angles every profile is sampled at.
    ///
    /// **All silhouettes share this sampling**, which is what makes any two
    /// shapes have points that correspond one to one — so a morph reduces to a
    /// linear interpolation of radii and there is no path-morphing library
    /// anywhere in this port. A new shape has to go through a radial profile,
    /// or through ``BloubShape/profile(fromPolygon:centerX:centerY:)``.
    public static let sampleCount = 64

    /// Egg: the ball's height, narrowed in width.
    /// Frame 164, measured footprint 1.647 x 2.000.
    public static let egg: [Double] = [
    ...
    /// Hexagon, point up, very round corners.
    /// Frame 174, measured footprint 1.826 x 2.011.
    public static let hexagon: [Double] = [
    ...
    /// Triangle, point up, very round corners.
    /// Frame 190, measured footprint 1.995 x 1.884.
    public static let triangle: [Double] = [
    ...
    /// The measured profiles by name, for ``BloubShape/silhouette(_:pose:)``.
    public enum Name: String, Sendable, CaseIterable {
        ...
        public var radii: [Double] {
            switch self {
            case .egg: BloubProfiles.egg
            case .hexagon: BloubProfiles.hexagon
            case .triangle: BloubProfiles.triangle
            }
        }
    }
}
