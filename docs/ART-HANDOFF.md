...
> **本节的"全局附属区"布局已被 §5.2 取代**（会议室 / 休息区现在挂在每个项目自己的 suite 里）。这里的 tile 表、素材规格和"角色在区域间走动"的设定**依然有效**，继续按它交付；只有"整张地图共用一间会议室 / 一个花园"这件事变了。
...
### 5.2 一个项目 = 一间小公司（当前形态，`feat/scene-company`）

§5.1 的"全局附属区"已经**被取代**：会议室和花园不再是整张地图共用的两条长条，而是**每个项目自己的房间**。一个项目 = 一个 **suite**（一间小公司），从上到下：

1. **工位区**（desks，尺寸和坐标与 §5.1 完全一致，Settings 全关时 suite 就等于它）；
2. **会议室**：每个正在 delegating 的家族一间，公司 ≥ 3 个 session 时即使没人开会也画一间空的；
3. **休息区**（break area）：**每个项目一间**，种类由项目路径 hash 定死（三选一，不随重启变），也可以在 Settings 里统一指定：
   - `garden` 花园（沿用 §5.1 花园 tile）
   - `teaRoom` 茶水间
   - `lounge` 休息室
4. suite 底部一条**走廊**（corridor），公司里所有房间都开向它；走廊右端是这家公司的**门**，ended 的 session 走到门口就**离开画面**（不再排队堆积）。

等你 QA 的人**分区看**：`!`（needs-you）和 `✓`（done）都坐在**自己项目休息区最靠走廊的那排长凳上**，不再是全局一条。

新增 tile（§5 / §5.1 通用表之外，命名不要改）：

| 归属 | name | 尺寸 | 备注 |
|---|---|---|---|
| 茶水间 | `counter` | 48×24 | 台面 + 下柜，台面上放 kettle / cups |
| 茶水间 | `kettle` | 8×12 | 壶嘴 + 把手，可加 §7 `coffeeSteam` |
| 茶水间 | `cups` | 16×8 | 两只倒扣的杯子 |
| 茶水间 | `fridge` | 16×32 | 双门，上冷冻下冷藏 |
| 茶水间 | `waterCooler` | 12×28 | 上桶下机身 + 出水口 |
| 茶水间 | `floorTileLight` | 16×16 | **四方连续无缝**，比 `floorTile` 亮一档 |
| 休息室 | `armchair` | 16×16 | 单人沙发，与 `sofa 48×32` 同布料 |
| 休息室 | `lamp` | 8×24 | 落地灯，灯罩亮 |
| 休息室 | `rugRound` | 32×32 | 圆地毯（3/4 俯视 = 椭圆），沙发 + 茶几踩在上面 |
| 休息室 | 复用 | — | `sofa 48×32`、`coffeeTable 32×20`、`bookshelf 32×48`、`plant 16×24`、`coffeeMachine 16×24` 已在 atlas 里，直接用 |
| suite 门 | `doorInner` | 16×32 | 室内门，**画成半开**（ended 从这里走出去）；与花园的 `gate 32×32` 并存，花园款 suite 用 `gate`，其余两款用 `doorInner` |
| suite 隔断 | `partition` | 16×24 | 公司内部隔断墙（工位区 / 会议室 / 休息区之间），实心 |
| suite 隔断 | `partitionGlass` | 16×24 | 同上，玻璃款，可透出后面地面 |

**当前实现状态**：以上全部由 `Sources/AuspexApp/Scene/PlaceholderArt.swift` 用代码画（`counter()` / `kettle()` / `cups()` / `fridge()` / `waterCooler()` / `sofa()` / `armchair()` / `coffeeTable()` / `bookshelf()` / `lamp()` / `rugRound()` / `doorInner()`），隔断还没画。和 §5.1 一样：**atlas loader 还没做**，交付的 tile 暂时不会上屏，但命名按本表来，loader 一到就整体替换。

摆放规则（app 侧已经这么摆，出图时按这个构图想）：
- 休息区家具全部贴**后墙线**站成一排（同一条脚底线，不是同一条顶线），人坐在它前面几排；地毯类躺在家具前面一点。
- 走廊是休息区最下面那条线，门在它的右端；门左边留 `gateReserve` 一格宽的空地，长凳不能占。
- 三种休息区的地面色各不相同（花园=草地、茶水间=浅地砖、休息室=暖地毯），这是**一眼认出是哪家公司**的主要线索，请保持三者在明度上拉开。

### 5.3 大公司的 suite 现在是"横的"，房间边界是临时画的线（`fix/ui-squeeze`）

两处变化，都会影响你出图时脑子里的构图：

1. **房间可能在工位区右边，不再一定在下面。** 一个项目超过一行工位、并且房间那一列比工位区矮时，会议室和休息区会站在工位区**右侧**：会议室在这一列的顶上，休息区在这一列的底部（贴走廊，因为门在那儿）。64 个 session 的公司因此从 832×1200 的长条变成 1100×862 的方块。小公司（一行工位）还是老样子——房间在下面。
2. **每个房间现在自己画了一圈虚线，整个 suite 外面画了一圈实线。** 这是**临时的代码画法**（`SceneRoomChrome`，`SKShapeNode` 画的虚线，离房间边 4 pt），目的是让"这三间屋子是一家公司"这件事在缩得很小的时候也看得出来。走廊也从一条细线变成了一条**压暗的地带**。

**这三样东西（房间虚线、suite 实线、走廊压暗）都是等你的 tile 来替换的占位。** 下面是"给我这些我就能把线全删掉"的清单，按优先级排；命名沿用 §5 / §5.2 的风格，16 px 网格、3/4 俯视、透明底、地面类四方连续无缝：

| 优先级 | name | 尺寸 | 用在哪 / 画什么 |
|---|---|---|---|
| 1 | `partition` | 16×24 | 已在 §5.2 列过，**还没交**。公司内部隔断：工位区 / 会议室 / 休息区之间那道墙。现在这道墙是虚线 |
| 1 | `partitionGlass` | 16×24 | 同上，玻璃款。会议室朝走廊那面用它，因为要看得见里面有没有人在开会 |
| 1 | `partitionCornerTL` `partitionCornerTR` `partitionCornerBL` `partitionCornerBR` | 16×24 | **新增**。房间现在会并排放，隔断要拐弯；只有直墙拼不出右边那一列 |
| 1 | `partitionEndCap` | 8×24 | **新增**。隔断断口（走廊穿过隔断的地方），断面要有厚度，不能是硬切 |
| 2 | `corridorRunner` | 16×16 | **新增**。走廊地面，四方连续无缝，明度比三种房间地面都低一档（现在是一条 30 % 黑的压暗带） |
| 2 | `corridorEdgeN` `corridorEdgeS` | 16×4 | **新增**。走廊和房间地面之间的过渡条（南北两侧各一条） |
| 2 | `wallOuterTop` | 16×16 | **新增**。suite 的外墙上沿（一家公司的外框上边），现在是一条实线 |
| 2 | `wallOuterSide` | 8×16 | **新增**。外墙左右边 |
| 2 | `wallOuterCornerTL` `wallOuterCornerTR` `wallOuterCornerBL` `wallOuterCornerBR` | 8×16 | **新增**。外墙四角 |
| 3 | `companyPlate` | 64×16 | **新增**。suite 顶边的公司名牌底板（字 app 写），比门牌 `roomLabelPlate 32×12` 大一号 |
| 3 | `roomLabelPlateWide` | 48×12 | **新增**。"Meeting rooms"（复数）这种放不下的门牌用 |
| 3 | `floorCarpetDark` / `floorTileLight` / `grass` | 16×16 | §5.1 / §5.2 已列过，**这三张一到，三个房间的低透明度底色就能换成真地面** |

出图时按这个构图想：一家公司 = 左边一片工位（最多 8 列，行数随人数长），右边一列房间（上会议室、下休息区），底下一条横穿整个公司的走廊，走廊右端是门。整个东西外面一圈外墙，房间之间是隔断。
