...
**Perch** 把同一批工作变成用户拥有的空间记忆，而不是机器自动排布的房间。每个根任务一张
卡，子 agent 折叠在下面；真实任务依赖使用实线箭头，观测到的 lineage 使用虚线。卡片
重启后仍在原位。`All boards` 永远包含全部工作；自定义 board 可以把同一任务镜像到另一
个位置，也可以用嵌套的项目、harness、标签、状态与 Attention 规则自动吸纳任务。

![Perch：项目分区里的用户摆放任务卡、任务依赖和当前选中项](docs/screenshots/map.png)

底部时间带按事件编号，而不是按墙上时间均匀分布。拖动后会重建当时的会话状态、任务状态、
board 成员和规则；卡片位置与相机保持当前空间记忆。History 只读；**Fork board here** 只复制视图并暂停规则，从不
回滚真实 Agent。视图分支通过三方比较合并，真正冲突的每一项都由人选择。
...
| **Flock**（原 Crew）—— 一个任务一个几何头像，表情和姿态由参与这项工作的会话驱动。信息量和 Aviary 相同，像素只用十分之一。 | **Flight**（原 Trajectory，⌘T）—— 把一个会话或整个 Task 摊开。**Graph** 播放 Agent 树、活跃 lineage 和进行中的工具 chips；**Trace** 保留事件瀑布与 Summary/Preview/Raw。两者共享 playhead、scope、速度与 Jump to Live。 |
